...
You must write a program which categorizes each flight listed in flight_data.csv into the most specific bucket found in the flight_buckets.csv file. It is important to note that due to flight purchase information coming from a variety of sources the casing of the strings contained in the file is not guaranteed to be consistent. Therefore all string comparisons should be done in a case-insensitive fashion. The rules for comparing the relative specificity of two buckets are as follows:
